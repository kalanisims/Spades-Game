...
export default function SpadesGame() {
  ...
  useEffect(() => {
    ...
    aiTimer.current = setTimeout(() => {
      ...
      const card = aiChooseCard(hand, trick, spadesBroken, bids, tricksWon);
  ...
  function playCard(playerIdx, card) {
    ...
    if (newTrick.length === 4) {
      // Determine winner
      const winner = newTrick.reduce((w, t) => beatsTrick(newTrick.slice(0, newTrick.indexOf(t)), t.card, newSpadesBroken) ? t : w, newTrick[0]);
      // Re-evaluate properly
      let winEntry = newTrick[0];
      for (let i = 1; i < newTrick.length; i++) {
        const prev = newTrick.slice(0, i);
        const isWin = (() => {
